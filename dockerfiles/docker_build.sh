...
{ DS=$(date +%s);
  docker build -t cuda-torch-deps1:$CUDA --build-arg CUDA=$CUDA cuda-torch/deps1
  echo "time: $(( $(date +%s) - DS )) seconds";
} 2>logs/cuda-torch-deps1-$CUDA.err >logs/cuda-torch-deps1-$CUDA.log;

### CUDA Torch dependencies 2 ###
{ DS=$(date +%s);
  docker build -t cuda-torch-deps2:$CUDA --build-arg CUDA=$CUDA cuda-torch/deps2
  echo "time: $(( $(date +%s) - DS )) seconds";
} 2>logs/cuda-torch-deps2-$CUDA.err >logs/cuda-torch-deps2-$CUDA.log;

### CUDA Torch ###
{ DS=$(date +%s);
  docker build -t cuda-torch:$CUDA --build-arg CUDA=$CUDA cuda-torch
  echo "time: $(( $(date +%s) - DS )) seconds";
...
{ DS=$(date +%s);
  docker build -t laia:${REV}-cuda$CUDA --build-arg CUDA=$CUDA .
  echo "time: $(( $(date +%s) - DS )) seconds";
