...
function createModel(sample_height, num_labels)
  ...
  model:add(nn.Reshape(-1, nm[4] * (sample_height / 8), true))
  model:add(nn.Dropout(0.5))
  model:add(cudnn.BiLSTM(nm[4] * (sample_height / 8), nh[1], 3, nil, 0.5))
  --model:add(cudnn.BGRU(nm[4] * (sample_height / 8), nh[1], 3, nil, 0.5))
  model:add(nn.Dropout(0.5))
  model:add(nn.Contiguous())  
