...
function MDRNN:splitGradOutput(output)
   ...
   output_c = output_c:permute(3, 1, 4, 2):contiguous()
   output_c = output_c:view(H, N * W, 2 * self.hiddenSize)
   ...
   output_r = output_r:permute(4, 1, 3, 2):contiguous()
   output_r = output_r:view(W, N * H, 2 * self.hiddenSize)
