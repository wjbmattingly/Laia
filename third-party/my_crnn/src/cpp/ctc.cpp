...
int forwardBackward(lua_State* L) {
  const typename thpp::Tensor<T>::Ptr input     = fblualib::luaGetTensorChecked<T>(L, 1);
  const typename thpp::Tensor<int>::Ptr targets = fblualib::luaGetTensorChecked<int>(L, 2);
  const bool forwardOnly          = lua_toboolean(L, 3);
  typename thpp::Tensor<T>::Ptr gradInput       = fblualib::luaGetTensorChecked<T>(L, 4);
