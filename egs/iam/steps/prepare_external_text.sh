...
for f in "${corpora[@]}"; do
  c="$(basename "$f")"; c="${c%.*}";
  txt="data/lang/word/external/${c}.txt";
  tok="data/lang/word/external/${c}_tokenized.txt";
  bnd="data/lang/word/external/${c}_boundaries.txt";
  ...
  [[ "$overwrite" = false && -s "data/lang/char/external/$c.txt" &&
      ( ! "data/lang/char/external/$c.txt" -ot "$txt" ) ]] ||
  awk -v ws="$wspace" '{
    for(i=1;i<=NF;++i) {
    ...
    printf("\n");
  }' "$txt" > "data/lang/char/external/$c.txt" ||
  ( echo "ERROR: Creating file data/lang/char/external/$c.txt" >&2 && exit 1 );
done;
