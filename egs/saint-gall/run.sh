...
[ -f data/saintgalldb-v1.0.zip ] || {
  [ -z "$FKI_USER" -o -z "$FKI_PASSWORD" ] &&
  echo "Please, set the FKI_USER and FKI_PASSWORD variables to download the" \
    "Saint-Gall database from the FKI servers." >&2 && exit 1;
  wget -P data --user="$FKI_USER" --password="$FKI_PASSWORD" \
    http://www.fki.inf.unibe.ch/DBs/iamHistDB/data/saintgalldb-v1.0.zip;
}

## Unzip dataset.
[ -d data/saintgalldb-v1.0 ] ||
( cd data && unzip -uqq saintgalldb-v1.0.zip && rm -rf __MACOSX && cd - &> /dev/null ) ||
exit 1;
